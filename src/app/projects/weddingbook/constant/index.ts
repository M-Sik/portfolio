...
export const troubleShootings = [
  {
    title: 'Jwt 로컬스토리지 기반 인증 → 쿠키 기반 인증 구조로 개선',
    contents: [
      {
        subTitle: '[문제 배경]',
        content:
      ...
      },
      {
        subTitle: '[해결 방법]',
        content:
          '로그인 시 서버에서 API 응답 값으로 받아오는 토큰을 Next.js 내부 서버로 전달하여 Access 토큰의 경우 Same Site(CSRF 방어) 옵션을 적용하여 쿠키로 저장. Refresh 토큰의 경우 쿠키에 Http Only(Js에서의 접근을 방지하여 XSS 방어), Same Site(CSRF 방어) 옵션을 적용하여 안전하게 관리되도록 처리하였습니다. 그리고 운영 환경에서는 쿠키에 Secure 옵션도 추가로 적용하여 주었습니다.',
      },
    ],
  },
  // {
